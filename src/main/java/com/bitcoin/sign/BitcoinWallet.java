...
public class BitcoinWallet implements Wallet {
                    ...
                    if (addressKey.containsKey(address)) {
                        ...
                        for (RawInput sigInput : signingTx.getInputs()) {
                            if (sigInput.getTxHash().equalsIgnoreCase(outpoint.getTransactionId())
                                ...
                                } else if ("BCH".equals(coinType) || "BCC".equals(coinType)) {
                                    byte[] hashTypeBytes = ByteUtilities.stripLeadingNullBytes(BigInteger.valueOf(65).toByteArray());
                                    hashTypeBytes = ByteUtilities.leftPad(hashTypeBytes, 4, (byte) 0x00);
                                    hashTypeBytes = ByteUtilities.flipEndian(hashTypeBytes);
    ...
    public String signTransaction(String address, String transaction, String privateKey, Outpoint[] outputs, String coinType, byte addressVersion) {

        //私钥生成对应地址
        //Map<String,String> addressKey = new HashMap<String,String>();
        //addressKey.put(BitcoinTools.getAddressByPrivateKey(privateKey,addressVersion),privateKey);

        SignedTransaction signedTransaction = new SignedTransaction();
        Iterable<Iterable<String>> signatureData = getSigString(transaction, privateKey, outputs, coinType, addressVersion);
        signatureData = signWithPrivateKey(signatureData, false);

        String trx = applySignature(address, transaction, signatureData, coinType);
        signedTransaction.setTransaction(trx);

        return signedTransaction.getTransaction();
    }

    private Iterable<Iterable<String>> getSigString(String transaction, String privateKey, Outpoint[] outputs, String coinType, byte addressVersion) {
        LinkedList<Iterable<String>> signatureData = new LinkedList<>();
            ...
            for (Outpoint output : outputs) {
                if (output.getTransactionId().equalsIgnoreCase(input.getTxHash())
                    ...
                    if (output.getAmount() != null) {
                        outpoint.setAmount(Long.valueOf(output.getAmount().multiply(new BigDecimal("100000000")).stripTrailingZeros().toPlainString()));
                    }
                    //String address = BitcoinTools.getAddressByScriptPuKey(output.getScriptPubKey(),addressVersion);
                    String address = output.getAddress();
                    //TODO 处理多签
                    ...
                    for (RawInput sigInput : signingTx.getInputs()) {
                        if (sigInput.getTxHash().equalsIgnoreCase(outpoint.getTransactionId())
                            ...
                            if ("BCH".equals(coinType)) {
                                sigInput.setAmount(outpoint.getAmount());
                                signingTx.setInput(sigInput);
                                sigString = signingTx.bchEncode();
                            } else {
                                sigString = signingTx.encode();
                            }
                            ...
                            System.out.println("序列化结果：" + sigString);
                            try {
                                sigData = ByteUtilities.toByteArray(sigString);
                                MessageDigest md = MessageDigest.getInstance("SHA-256");
                                sigData = md.digest(md.digest(sigData));
     ...
     * @return
     */
    public String applySignature(String address, String transaction, Iterable<Iterable<String>> signatureData, String coinType) {
