...
public class RawTransaction {
  ...
  public void setInput(RawInput input) {
    this.input = input;
  }
  ...
  public String bchEncode() {
    ...
    String inputStr = "";
    for (int i = 0; i < getInputCount(); i++) {
      inputStr = inputStr + getInputs().get(i).encodeSimple();
    }
    MessageDigest md = null;
    try {
      md = MessageDigest.getInstance("SHA-256");
      md.reset();
      md.update(ByteUtilities.toByteArray(inputStr));
      byte[] inputHash = md.digest();
      md = MessageDigest.getInstance("SHA-256");
      md.reset();
      md.update(inputHash);
      inputHash = md.digest();
      tx.append(ByteUtilities.toHexString(inputHash));
    } catch (Exception e) {
      e.printStackTrace();
    }
    ...
    for (int i = 0; i < getInputCount(); i++) {
      byte[] sequenceBytes =
              ByteUtilities.stripLeadingNullBytes(BigInteger.valueOf(getInputs().get(i).getSequence()).toByteArray());
      sequenceBytes = ByteUtilities.leftPad(sequenceBytes, 4, (byte) 0xFF);
      sequenceBytes = ByteUtilities.flipEndian(sequenceBytes);
      sequenceStr = sequenceStr + ByteUtilities.toHexString(sequenceBytes);
    }
    try {
      md = MessageDigest.getInstance("SHA-256");
      md.reset();
      md.update(ByteUtilities.toByteArray(sequenceStr));
      byte[] sequenceHash = md.digest();
      md = MessageDigest.getInstance("SHA-256");
      md.reset();
      md.update(sequenceHash);
      sequenceHash = md.digest();
      tx.append(ByteUtilities.toHexString(sequenceHash));
    } catch (Exception e) {
      e.printStackTrace();
    }

    //序列化自己的当前input
    tx.append(getInput().bchEncode());

    // 序列化Outputs
    String outputStr = "";
    for (int i = 0; i < getOutputCount(); i++) {
      outputStr = outputStr + getOutputs().get(i).encode();
    }
    try {
      md = MessageDigest.getInstance("SHA-256");
      md.reset();
      md.update(ByteUtilities.toByteArray(outputStr));
      byte[] outputHash = md.digest();
      md = MessageDigest.getInstance("SHA-256");
      md.reset();
      md.update(outputHash);
      outputHash = md.digest();
      tx.append(ByteUtilities.toHexString(outputHash));
    } catch (Exception e) {
      e.printStackTrace();
    }
